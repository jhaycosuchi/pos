...
export default function MenuParaLlevar() {
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-gray-800/95 backdrop-blur-md shadow-xl p-4 border-b border-gray-700">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Pedido Para Llevar</h1>
            <button
          ...
          <div className="flex overflow-x-auto p-2 space-x-2">
            {categorias.map((categoria, idx) => (
              <button
                ...
                className={`px-6 py-3 rounded-lg whitespace-nowrap font-bold transition-all transform hover:scale-105 ${
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-gray-900">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {categoriaActual?.items.map((item, idx) => (
              <div
                key={item.nombre + idx}
                className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden transition-all transform hover:scale-105 hover:shadow-2xl hover:shadow-orange-500/20 cursor-pointer group"
              ...
              >
                ...
                <div className="p-4">
                  <h3 className="font-bold text-white mb-2 text-lg">{item.nombre}</h3>
                  {item.descripcion && (
                    <p className="text-sm text-gray-400 mb-3 line-clamp-2">{item.descripcion}</p>
                  )}
                  <div className="flex items-center justify-between">
                    <span className="text-2xl font-bold text-orange-500">
                      ${item.precio.toFixed(2)}
                    </span>
                    <span className="text-xs text-gray-500 group-hover:text-orange-400 transition">+ Agregar</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="w-96 bg-gray-800 border-l border-gray-700 shadow-2xl flex flex-col">
        {/* Header del carrito */}
        <div className="p-4 border-b border-gray-700 bg-gradient-to-r from-orange-900/30 to-gray-800">
          <h2 className="text-xl font-bold text-white">Carrito</h2>
          <p className="text-sm text-orange-400 font-medium">Para Llevar</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-gray-900">
          {cart.length === 0 ? (
            <div className="text-center text-gray-500 mt-8">
              ...
              <p className="text-sm mt-2 text-gray-600">Selecciona items del menú para agregar</p>
            </div>
          ) : (
            <div className="space-y-3">
              {cart.map((cartItem) => (
                <div key={cartItem.id} className="bg-gray-800 rounded-lg p-3 border border-gray-700 shadow-md">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <h4 className="font-bold text-white">{cartItem.item.nombre}</h4>
                      <p className="text-sm text-gray-400">${cartItem.item.precio.toFixed(2)} c/u</p>
                    </div>
                    <button
                      onClick={() => eliminarDelCarrito(cartItem.id)}
                      className="text-red-500 hover:text-red-400 ml-2 text-xl font-bold"
                      title="Eliminar"
                    >
                      ✕
                    </button>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <button
                        ...
                        className="w-8 h-8 bg-gray-700 rounded-full hover:bg-gray-600 transition font-bold text-white"
                      >
                        -
                      </button>
                      <span className="w-12 text-center font-bold text-white text-lg">{cartItem.quantity}</span>
                      <button
                        onClick={() => actualizarCantidad(cartItem.id, cartItem.quantity + 1)}
                        className="w-8 h-8 bg-orange-600 rounded-full hover:bg-orange-500 transition font-bold text-white"
                      >
                        +
                      </button>
                    </div>
