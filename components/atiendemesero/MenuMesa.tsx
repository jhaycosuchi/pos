/**
 * Componente de menú para una mesa específica - OPTIMIZADO PARA MÓVIL
...
export default function MenuMesa({ mesaNumero }: MenuMesaProps) {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-900 text-white md:flex-row">
      {/* Panel izquierdo - Menú (Completo en móvil) */}
      <div className="flex-1 flex flex-col md:w-3/4 overflow-hidden">
        ...
        <div className="bg-gray-800/95 backdrop-blur-md shadow-xl p-2 sm:p-3 border-b border-gray-700 flex-shrink-0">
          <div className="flex items-center justify-between gap-2">
            <div>
              <h1 className="text-base sm:text-lg font-bold text-white truncate">Mesa {mesaNumero}</h1>
        ...
        <div className="flex-1 overflow-y-auto p-1.5 sm:p-2 bg-gray-900">
          <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
            {categoriaActual?.items.map((item, idx) => (
              <div
                key={item.nombre + idx}
                className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 overflow-hidden transition-all transform hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/20 cursor-pointer group aspect-square flex flex-col"
      ...
      <div className="w-full md:w-1/4 bg-gray-800 border-t md:border-t-0 md:border-l border-gray-700 shadow-2xl flex flex-col md:h-screen">
        {/* Header del carrito */}
        <div className="p-2 sm:p-3 border-b border-gray-700 bg-gradient-to-r from-blue-900/30 to-gray-800 flex-shrink-0">
          <h2 className="text-base sm:text-lg font-bold text-white">Carrito</h2>
          <p className="text-xs sm:text-sm text-blue-400 font-medium">🍽️ Mesa {mesaNumero}</p>
        </div>
              ...
              {cart.map((cartItem) => (
                <div key={cartItem.id} className="bg-gray-800 rounded-lg p-2 border border-gray-700 shadow-md">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center space-x-1">
                      ...
                      <button
                        ...
                        className="w-6 h-6 bg-blue-600 rounded-full hover:bg-blue-500 transition font-bold text-white text-xs"
                      >
                        +
                      </button>
                    </div>
                    <div className="font-bold text-blue-500 text-xs flex-shrink-0">
                  ...
                  {cartItem.options && cartItem.options.notaPersonalizada && (
                    <div className="mt-1.5 text-xs text-gray-600">
                      <p className="font-medium">Nota: {cartItem.options.notaPersonalizada}</p>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="border-t border-gray-700 p-2 sm:p-3 space-y-2 bg-gray-800 flex-shrink-0">
          ...
          <textarea
            ...
            className="w-full p-2 text-xs bg-gray-700 border border-gray-600 rounded text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-none"
            rows={2}
          />
