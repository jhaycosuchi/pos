...
interface EditMeseroFormProps {
  mesero: Mesero;
}

export default function EditMeseroForm({ mesero }: EditMeseroFormProps) {
  const router = useRouter();
  const [formData, setFormData] = useState({
    nombre: mesero.nombre,
    username: mesero.username,
    rol: mesero.rol,
    activo: mesero.estado,
    password: '',
    confirmPassword: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/pos/api/usuarios/${mesero.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        ...
        setError(data.message || 'Error al actualizar el mesero');
        setLoading(false);
        return;
      }

      const data = await response.json();
      setSuccess('Mesero actualizado exitosamente');
